...
namespace Grapheme {
	CTranscriptSerializerImpl::CTranscriptSerializerImpl(const SSerializationConfig& a_config)
		: m_config(a_config) {

	}

	EResultCode CTranscriptSerializerImpl::SaveToFile(const std::string& a_path, const STranscriptResult& a_result) const {
	...
	EResultCode CTranscriptSerializerImpl::SaveToFile(const std::string& a_path, const std::vector<STranscriptResult>& a_results) const {
		std::ofstream file(a_path);
		if (!file.is_open()) {
			return EResultCode::InternalError;
		}
	...
	STranscriptResult CTranscriptSerializerImpl::LoadFromFile(const std::string& a_path) const {
	...
	std::string CTranscriptSerializerImpl::Serialize(const STranscriptResult& a_result) const {
		nlohmann::json json;
	...
	std::string CTranscriptSerializerImpl::Serialize(const SLanguageResult& a_result) const {
		nlohmann::json json;
	...
	std::string CTranscriptSerializerImpl::Serialize(const std::vector<STranscriptResult>& a_results) const {
	...
	STranscriptResult CTranscriptSerializerImpl::DeserializeTranscriptResult(const std::string& a_json_string) const {
		STranscriptResult result;
		try {
	...
	SLanguageResult CTranscriptSerializerImpl::DeserializeLanguageResult(const std::string& a_json_string) const {
